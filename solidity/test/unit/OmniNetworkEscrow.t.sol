...
abstract contract Base is Test {
  address internal _owner = vm.addr(1); // owner AND relayer
...
contract UnitListing is Base {
  function testListingRevertIfNotOwner() public {
    ...
    _escrow.listXERC20Token(address(_xerc20), block.timestamp + 100, 1, '', '');
  }
  ...
  function testListingRevertIfTimestampIsNotInTheFuture() public {
    ...
    _escrow.listXERC20Token(address(_xerc20), block.timestamp, 1, '', '');
    vm.stopPrank();
  }
  ...
  function testListingRevertIfAmountToClaimIsZero() public {
    ...
    _escrow.listXERC20Token(address(_xerc20), block.timestamp + 1, 0, '', '');
    vm.stopPrank();
  }
  ...
  function testListing() public {
    ...
    (, uint256 claimDeadline, uint256 totalClaimable, string memory _campaignId, uint256 totalClaimedWallets,) =
      _escrow.listedTokenDetails(address(_xerc20));

    assertEq(claimDeadline, block.timestamp + 100);
    assertEq(totalClaimable, 1);
    assertEq(_campaignId, '');
...
contract CollectUnitTest is Base {
  function testCollectRevertIfAlreadyClaimed() public {
    ...
    // First claim
    vm.startPrank(_owner);
    _escrow.collectXERC20(address(_xerc20), _user);
    vm.stopPrank();
