...
contract UnitListing is Base {
    ...
    function testListingRevertIfTimestampIsNotInTheFuture() public {
        vm.startPrank(_owner);
        vm.expectRevert(
            OmniNetworkEscrow.OmniEscrow_DeadlineMustBeInTheFuture.selector
        );
    ...
    function testListingRevertIfAmountToClaimIsZero() public {
        vm.startPrank(_owner);
        vm.expectRevert(
            OmniNetworkEscrow.OmniEscrow_TotalClaimableBiggerThanZero.selector
        );
...
contract CollectUnitTest is Base {
    ...
    function testCollectSuccess() public {
        ...
        (, uint256 totalClaimable, , uint256 totalClaimedWallets) = _escrow
            .listings(address(_xerc20));
        ...
        (, , , uint256 totalClaimedWalletsAfter) = _escrow.listings(
            address(_xerc20)
        );

        uint256 timestampClaim = _escrow.claimedWallets(
            address(_xerc20),
            _user
        );
        ...
        assertEq(
            IERC20(_xerc20).balanceOf(_user),
            balanceTokenBefore + totalClaimable
        );
